> [官方文档]([Git - git-reflog Documentation](https://git-scm.com/docs/git-reflog/zh_HANS-CN))
___
...
引用日志（或称 "reflogs"）记录了本地仓库中分支和其他引用的提示更新时间。参考日志在各种 Git 命令中都很有用，可以用来指定引用的旧值。例如，`HEAD@{2}` 表示 “两步前 HEAD 所在的位置”，`master@{one.week.ago}` 表示 “一周前本仓库中 master 所在的位置”，等等。详见 [gitrevisions[7]](https://git-scm.com/docs/gitrevisions/zh_HANS-CN)。

该命令采取不同的子命令，并根据子命令有不同的选项：

"show" 子命令（也是默认命令，在没有任何子命令的情况下）显示命令行中提供的引用日志（或默认的 `HEAD`）。引用日志包括所有最近的操作，此外，`HEAD` 引用日志还记录分支切换。 `git reflog show` 是 `git log -g --abbrev-commit --pretty=oneline` 的别名；详见 [git-log[1]](https://git-scm.com/docs/git-log/zh_HANS-CN)。

"list" 子命令列出了所有具有相应引用日志的引用。

"expire" 子命令用于删除旧的引用日志条目。超过 `expire` 时间的条目，或超过 `expire-unreachable` 时间且当前 tip 无法访问的条目，都会从引用日志中删除。 终端用户通常不会直接使用该子项，请参见 [git-gc[1]](https://git-scm.com/docs/git-gc/zh_HANS-CN)。
...
这种计算涉及遍历所有可到达的对象，也就是说，其代价与 _git prune_ 相同。 它的主要目的是修复旧版本 Git 垃圾回收所造成的损坏，因为旧版本的 Git 无法保护引用日志所引用的对象。
